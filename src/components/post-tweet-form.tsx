...
export default function PostTweetForm() {
    ...
    const[file,setFile]=useState<File|null>(null); //it will be file or null
    ...
    const onFileChange=(e:React.ChangeEvent<HTMLInputElement>)=>{
        const { files } =e.target; //similar to create-account {target:{name,value}}
    ...
    const onSubmit=async(e:React.FormEvent<HTMLFormElement>)=>{
        ...
        try{
            ...
            const doc=await addDoc(collection(db,"tweets"),{
                ...
                userId: user.uid, //삭제처리 위해서. 이름은 같을 수 있으니까.

            });
            if(file){
                const locationRef = ref(storage, `tweets/${user.uid}-${user.displayName}/${doc.id}`)
                const result= await uploadBytes(locationRef, file); //uploadBytes(where, what)
